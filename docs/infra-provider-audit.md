...
- Cloudflare Access is Terraform-wired as an opt-in setting for QA Admin. A trial apply failed with Cloudflare API `403 Authentication error` even though the token verifies as active, so the local `CLOUDFLARE_API_TOKEN` needs Zero Trust Access application write permission before `cloudflare_access_enabled = true` can be applied. WAF/ruleset resources remain under review because their policy shape is security-sensitive and depends on final traffic behavior.
