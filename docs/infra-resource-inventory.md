...
| Area | Status | Notes |
| --- | --- | --- |
| Vercel | in progress | Storefront project `medusa-store-storefront` is imported into shared Terraform. It is intentionally not linked to Git; manual GitHub Actions dispatch owns deploys. QA preview `MEDUSA_BACKEND_URL` is Terraform-managed. Domains and remaining app environment variables are pending until final domain/public app config values exist. |
