...
| Area | Status | Notes |
| --- | --- | --- |
| Vercel | in progress | Separate QA and production storefront projects are Terraform-managed. Both projects are intentionally not linked to Git; manual GitHub Actions dispatch owns deploys. QA app config is Terraform-managed on the QA project. Production has the storefront domains and browser-safe image hostnames only; paid/runtime production backend config remains deferred. |
| Railway | pending | Replace/delete any app hosting, databases, Redis, variables, domains, and workflows if present. |
| Cloudflare | in progress | Terraform manages `neonfold.com` DNS for apex, `www`, `qa`, and `qa-api`, plus R2 media buckets/custom domains for QA and production media. R2 custom-domain ownership and SSL are active. Access/WAF/Turnstile/Web Analytics remain under review before wiring. |
| AWS | in progress | Terraform state bucket created in `ap-southeast-1`; obsolete DynamoDB lock table removed after switching to native S3 lockfiles. Production Lightsail resources were removed after the QA-first sequencing decision. QA Lightsail, QA SSM runtime parameters, no-cost production SSM scaffolding, and GitHub Actions OIDC/QA/prod SSM read IAM are Terraform-managed. Continue inventory for billing alerts. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Vercel | `medusa-store-storefront-qa` project | qa | global functions `sin1` | Terraform shared | keep/import | Existing project ID `prj_q6k8F047LaG3b3DdPG4VItAC1HV9` was imported into Terraform on 2026-05-24 and renamed for QA. It uses Next.js, Node 24.x, Singapore function region, and no Git repository link so manual GitHub Actions dispatch controls deploys. | Keep under shared Terraform. Deploy QA with `vercel deploy --prod` against the QA project. | no |
| Vercel | `medusa-store-storefront-prod` project | prod | global functions `sin1` | Terraform shared | keep/import | Production project ID `prj_jOsdxviaTo8heZOl4wrFYOGIzlWP` is Terraform-managed. It is ready for domain/config scaffolding without production backend runtime cost. | Keep under shared Terraform. Deploy only from the protected production GitHub environment after merge to `main`. | no |
| Vercel | QA project environment variables | qa | global | Terraform shared | keep/import | `MEDUSA_BACKEND_URL`, `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY`, and `NEXT_PUBLIC_IMAGE_HOSTNAMES` target the QA project's production environment. `MEDUSA_BACKEND_URL` points to `https://qa-api.neonfold.com`. | Keep under shared Terraform; rotate publishable keys by creating/revoking keys in Medusa and updating Terraform. | no |
| Vercel | Production project environment variables | prod | global | Terraform shared | keep/import | `NEXT_PUBLIC_IMAGE_HOSTNAMES` targets the production project's production environment. Production backend app config remains deferred until production backend setup is explicitly approved. | Keep browser-safe shared values under Terraform; add production backend values only when production runtime is intentionally enabled. | no |
| Vercel | `qa.neonfold.com` project domain | qa | global | Terraform shared | keep/import | Attached to the QA Vercel storefront project. | Keep under shared Terraform and deploy QA manually from `dev`. | no |
| Vercel | `www.neonfold.com` project domain | prod | global | Terraform shared | keep/import | Attached to the production Vercel storefront project as the future production storefront domain. | Keep under shared Terraform; production deployment remains deferred. | no |
| Vercel | `neonfold.com` apex redirect | prod | global | Terraform shared | keep/import | Terraform-managed 308 redirect to `www.neonfold.com` on the production Vercel project. The module models redirects separately so the redirect target is created first. | Keep under shared Terraform. | no |
...
| Cloudflare | `neonfold.com` zone DNS records | shared | global | Terraform shared | keep/import | Domain was purchased in Cloudflare and is the accepted interim launch domain. Terraform manages apex `A` to Vercel, `www` CNAME to Vercel, `qa` CNAME to Vercel, and `qa-api` A to the QA Lightsail static IP. | Keep under shared Terraform; update the explicit QA/prod domain variables when moving to the final brand domain. | no |
| Cloudflare | `ecom-qa-media` R2 bucket and `qa-media.neonfold.com` custom domain | qa | global | Terraform shared | keep/import | Low/no-cost QA media storage and public media hostname. Custom-domain ownership and SSL are active. | Create scoped R2 S3 credentials manually and store them in QA SSM before enabling Medusa R2 file storage. | no |
| Cloudflare | `ecom-prod-media` R2 bucket and `media.neonfold.com` custom domain | prod | global | Terraform shared | keep/import | Low/no-cost production media storage and public media hostname. Custom-domain ownership and SSL are active. Empty bucket does not create production compute/runtime cost. | Create scoped R2 S3 credentials manually and store them in production SSM before enabling production media storage. | no |
...
| AWS | `ecom-qa-medusa-static-ip` Lightsail static IP | qa | ap-southeast-1 | Terraform qa | keep/import | Static origin IP for QA Medusa API/admin DNS. Current IPv4 is `52.77.164.161`, attached to `ecom-qa-medusa`, and used by `qa-api.neonfold.com`. | Keep for QA API until QA compute is intentionally replaced. | no |
...
| AWS | `/ecom/qa/medusa/STORE_CORS` SSM parameter | qa | ap-southeast-1 | Terraform qa | keep/import | Non-secret allowed storefront origins for QA, including `https://qa.neonfold.com` plus local/IP bootstrap origins. | Keep under QA Terraform; consumed by QA deploy env-file generation. | no |
| AWS | `/ecom/qa/medusa/ADMIN_CORS` SSM parameter | qa | ap-southeast-1 | Terraform qa | keep/import | Non-secret allowed admin/API origins for QA, including `https://qa-api.neonfold.com` plus local/IP bootstrap origins. | Keep under QA Terraform; consumed by QA deploy env-file generation. | no |
| AWS | `/ecom/qa/medusa/AUTH_CORS` SSM parameter | qa | ap-southeast-1 | Terraform qa | keep/import | Non-secret allowed auth origins for QA, including `https://qa-api.neonfold.com` and `https://qa.neonfold.com` plus local/IP bootstrap origins. | Keep under QA Terraform; consumed by QA deploy env-file generation. | no |
| AWS | `/ecom/qa/medusa/MEDUSA_BACKEND_URL` SSM parameter | qa | ap-southeast-1 | Terraform qa | keep/import | Non-secret external Medusa backend URL; value is `https://qa-api.neonfold.com`. | Keep under QA Terraform; redeploy QA Medusa so Caddy and runtime env consume it. | no |
...
| AWS | `/ecom/prod/medusa/NODE_ENV` SSM parameter | prod | ap-southeast-1 | Terraform prod | keep/import | No-cost production runtime scaffold; Standard String parameter, value is `production`. | Keep under prod Terraform; production deploy will consume it later. | no |
...
- Final brand domain later. Interim domain is `neonfold.com` with explicit QA/prod Terraform variables for `www.neonfold.com`, `neonfold.com`, `qa.neonfold.com`, `qa-api.neonfold.com`, `media.neonfold.com`, and `qa-media.neonfold.com`.
- AWS account/profile, state bucket name, and state region.
- Cloudflare admin email allowlist for future Access setup.
- Vercel token for Terraform and deployment. Current token resolves to the personal Anshul Bansal account context. QA project ID is `prj_q6k8F047LaG3b3DdPG4VItAC1HV9`; production project ID is `prj_jOsdxviaTo8heZOl4wrFYOGIzlWP`. Do not use Subscribili.
