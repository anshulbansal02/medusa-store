...
| Area | Status | Notes |
| --- | --- | --- |
| Vercel | in progress | Storefront project `medusa-store-storefront` is imported into shared Terraform. It is intentionally not linked to Git; manual GitHub Actions dispatch owns deploys. Domains and app environment variables remain pending until final domain/public app config values exist. |
...
| GitHub | `qa` environment variables | qa | global | GitHub CLI/manual | keep/import | `AWS_DEPLOY_ROLE_ARN`, `QA_LIGHTSAIL_TAILSCALE_HOST`, `QA_LIGHTSAIL_SSH_KNOWN_HOSTS`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, and `MEDUSA_BACKEND_URL` are set for the QA deploy path. `MEDUSA_BACKEND_URL` now points to the Lightsail static IP instead of Railway. | Keep deploy-only values here. Move stable storefront app environment values into Terraform-managed Vercel config once final values exist. | no |
