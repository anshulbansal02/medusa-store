...
| Area | Status | Notes |
| --- | --- | --- |
| Vercel | in progress | Storefront project `medusa-store-storefront` is imported into shared Terraform. It is intentionally not linked to Git; manual GitHub Actions dispatch owns deploys. QA preview `MEDUSA_BACKEND_URL` and `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` are Terraform-managed. Domains and remaining app environment variables are pending until final domain/public app config values exist. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Vercel | `medusa-store-storefront` project | QA/prod | global functions `sin1` | Terraform shared | keep/import | Existing project ID `prj_q6k8F047LaG3b3DdPG4VItAC1HV9` was imported into Terraform on 2026-05-24. It uses Next.js, Node 24.x, Singapore function region, and no Git repository link so manual GitHub Actions dispatch controls deploys. | Keep under shared Terraform. Add domains and environment variables through Terraform after final domain/public config values are known. | no |
| Vercel | `MEDUSA_BACKEND_URL` preview environment variable | qa | global | Terraform shared | keep/import | Vercel project env var ID `iutVpAQXldOq5Yqu`; target is `preview`; value points to the QA Medusa static IP URL. | Keep under shared Terraform; replace with QA API domain after domain setup. | no |
