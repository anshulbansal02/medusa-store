...
| GitHub | in progress | QA environment now has Lightsail deploy variables for AWS OIDC role, Tailscale host, storefront backend URL, pinned SSH known-hosts, Tailscale federated identity secrets, and the QA SSH deploy key. Obsolete Railway QA secret was removed. Continue with branch protection inventory. |
...
| Tailscale | `tag:ci` ACL policy | qa | global | Tailscale admin | keep/import | Tailnet policy allows GitHub Actions nodes tagged `tag:ci` to reach QA Lightsail Tailscale IP `100.71.144.128` on TCP 22. | Keep least-privilege; do not restore wildcard tailnet access for CI. | no |
