...
| Area | Status | Notes |
| --- | --- | --- |
| Vercel | in progress | Storefront project `medusa-store-storefront` is imported into shared Terraform. It is intentionally not linked to Git; manual GitHub Actions dispatch owns deploys. QA preview `MEDUSA_BACKEND_URL`, `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY`, and `NEXT_PUBLIC_IMAGE_HOSTNAMES` are Terraform-managed. `qa.neonfold.com`, `www.neonfold.com`, and the `neonfold.com` apex redirect are attached to the project. Remaining app environment variables are pending until public app config values exist. |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Vercel | `medusa-store-storefront` project | QA/prod | global functions `sin1` | Terraform shared | keep/import | Existing project ID `prj_q6k8F047LaG3b3DdPG4VItAC1HV9` was imported into Terraform on 2026-05-24. It uses Next.js, Node 24.x, Singapore function region, and no Git repository link so manual GitHub Actions dispatch controls deploys. | Keep under shared Terraform. Manage domains and environment variables through Terraform. | no |
| Vercel | `MEDUSA_BACKEND_URL` preview environment variable | qa | global | Terraform shared | keep/import | Vercel project env var ID `iutVpAQXldOq5Yqu`; target is `preview`; value points to `https://qa-api.neonfold.com`. | Keep under shared Terraform; redeploy QA storefront after Cloudflare DNS and QA backend HTTPS are ready. | no |
| Vercel | `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` preview environment variable | qa | global | Terraform shared | keep/import | Browser-safe QA Medusa publishable API key, retrieved from the QA Medusa database. Target is `preview`. | Keep under shared Terraform; rotate by creating/revoking publishable keys in Medusa and updating Terraform. | no |
| Vercel | `NEXT_PUBLIC_IMAGE_HOSTNAMES` preview/production environment variables | QA/prod | global | Terraform shared | keep/import | Browser-safe image hostname allow-list derived from media hostnames. Preview env var ID `eH7CP6QFDXwKpv7t`; production env var ID `EvsQWC7oaDfSybu5`. | Keep under shared Terraform; update by changing domain/subdomain variables. | no |
| Vercel | `qa.neonfold.com` project domain | qa | global | Terraform shared | keep/import | Attached to the Vercel storefront project for dev-branch QA preview deployments. | Create Cloudflare DNS record and redeploy QA storefront after DNS is active. | no |
| Vercel | `www.neonfold.com` project domain | prod | global | Terraform shared | keep/import | Attached to the Vercel storefront project as the future production storefront domain. | Create Cloudflare DNS record now; production deployment remains deferred. | no |
| Vercel | `neonfold.com` apex redirect | prod | global | Terraform shared | keep/import | Terraform-managed 308 redirect to `www.neonfold.com`. The module models redirects separately so the redirect target is created first. | Keep under shared Terraform. | no |
...
- Final brand domain later. Interim domain is `neonfold.com` with `www.neonfold.com`, `qa.neonfold.com`, and `qa-api.neonfold.com`.
- AWS account/profile, state bucket name, and state region.
- Cloudflare admin email allowlist for future Access setup.
- Vercel token for Terraform and deployment. Current token resolves to the personal Anshul Bansal account context and project `prj_q6k8F047LaG3b3DdPG4VItAC1HV9`; do not use Subscribili.
